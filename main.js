// Create a reference for the canvas

canvas=document.getElementById("myCanvas");
ctx=canvas.getContext("2d");

img_width = 300;
img_height = 100;

var img_image;

img_x = 100;
img_y = 100;

function add() {
	img_imgTag = new Image(); //defining a variable with a new image
	img_imgTag.onload = uploadimg; // setting a function, onloading this variable
	img_imgTag.src = img_image;   // load image
}

function uploadimg() {

	ctx.drawImage(img_imgTag, 0,0, canvas.width, canvas.height);
}

//Write a code to grab the key-pressed event
window.addEventListener("keydown", my_keydown)

function my_keydown(e)
{
	keyPressed = e.keyCode;
	console.log(keyPressed);
	
		if((keyPressed >=97 && keyPressed<=122)|| (keyPressed >=65 && keyPressed<=90)){
			aplhabetkey();
			document.getElementById("d1").innerHTML="You have pressed Alphabet Key";
			console.log("Alphabet Key")
		}
		//write a code to check the type of key pressed
		else if((keyPressed>=48 && keyPressed<=57)){
			numberkey();
			document.getElementById("d1").innerHTML="You have pressed Number Key";
			console.log("Alphabet Key")
		}
		else if((keyPressed>=37 && keyPressed<=40)){
			arrowkey();
			document.getElementById("d1").innerHTML="You have pressed Arrow Key";
			console.log("Arrow Key")
		}
		else if((keyPressed==17)|| (keyPressed==18|| keyPressed==27)){
			specialkey();
			document.getElementById("d1").innerHTML="You have pressed ctrl/esc/alt Key";
			console.log("Special Key")
		}
		else if((keyPressed==32)){
			spacebar();
			document.getElementById("d1").innerHTML="You have pressed Space Bar";
			console.log("Space Bar")
		}
	else{
		otherkey();
		document.getElementById("d1").innerHTML="You pressed symbol or other key";
	}
}

function aplhabetkey()
{
	//upload respective image with the message.
	img_image="alpha.png" 
	add();
}
function numberkey()
{
	img_image="number.png" 
	add();	
}
function arrowkey()
{
	img_image="arrow.png" 
	add();
}
function specialkey()
{
	img_image="special.png" 
	add();	
}
function spacebar()
{
	img_image="spacebar.png";
	add();
}
function otherkey()
{
	img_image="other .png";
	add();
}
	
